...
plot.per.base.sequence.content <- function(data_frame){
 plot <- ggplot(melt(per_base_sequence_content,id.vars="base"), aes(x = base, y = value, fill =variable )) +
...
plot.per.base.N.count <- function(data_frame){
  ggplot(data_frame,aes(base,N_count)) +
    geom_bar(stat = "identity",width=.7) +
...
plot.seq.duplication.levels <- function(data_frame){
  ggplot(melt(data_frame,id.vars=c("duplication_level")),aes(duplication_level,value,fill=as.factor(variable))) +
...
plot.per.sequence.quality.scores <- function(data_frame){
  ggplot(data_frame,aes(quality,count)) +
    geom_bar(stat="identity",width=.5)
}

plot.per.base.sequence.quality <- function(data_frame){
  ggplot(data_frame,aes(base,mean)) +
